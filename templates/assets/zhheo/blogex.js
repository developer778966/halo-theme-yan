...
// 生成标签云
function generateWordCloud(selector, data) {
    const screenWidth = window.innerWidth;
    const parentDiv = d3.select(".tags-container");
    const parentWidth = parentDiv.node().getBoundingClientRect().width;
    const svgWidth = screenWidth >= 768 ? parentWidth : 400;
    const svgHeight = 500;
    ...
    const layout = d3.layout.cloud()
        ...
        .words(words)
        .padding(15)
        .rotate(() => ~~(Math.random() * 60 - 30)) // 更改为与原代码一致的角度旋转
        .fontSize(d => Math.max(10, (d.size - 0) * 10 + 10))
        .on("end", draw);
    ...
    function draw(words) {
        svg.append("g")
            ...
            //.attr("target", "_blank")
            .append("text")
            .style("font-size", d => `${d.size}px`) // 与 fontSize 方法保持一致
            .style("fill", d =>  generateColor()) // 使用默认颜色生成函数
    ...
    // .chip标签元素设置随机背景色
    document.querySelectorAll(".tags-content .chip").forEach(t => {
      t.style.backgroundColor = generateColor()
    }
    )
...
function initCategoryRadarChart() {
    const chartContainer = document.getElementById("category-echarts");
    if (!chartContainer) return;

    const dataValues = Object.values(categoryRadar);
    const dataKeys = Object.keys(categoryRadar);

    if (dataKeys.length < 3) return;

    chartContainer.classList.add("category-echarts");
    const chartInstance = echarts.init(chartContainer);
    const backgroundColor = document.querySelectorAll(".dark").length > 0 ? "#ccc" : "black";
    ...
    chartInstance.setOption({
        ...
        radar: [{
            ...
            indicator: dataKeys.map(key => ({
                name: key,
                max: Math.ceil(Math.max(...dataValues) / 5) * 5 // 计算最大值
            })),
            center: ["50%", "60%"],
            radius: "60%"
        }],
    ...
    document.querySelectorAll(".categories-container .chip").forEach(t => {
        t.style.backgroundColor || (t.style.backgroundColor = generateColor())
    }
    )
}
